...
def generate_batches(Gs, use_node_labels, n_feat, y, batch_size, radius, device, shuffle=False):
    ...
    for i in range(0, N, batch_size):
            ...
            for k,n1 in enumerate(Gs[index[j]].nodes()):
                ...
                for n2 in ego.nodes():
                    for n3 in ego.neighbors(n2):
                        if dists[n3] == dists[n2]+1:
                            edges_batch[dists[n2]].append((tuple_to_idx[dists[n2]][(n1,n2)], tuple_to_idx[dists[n2]+1][(n1,n3)]))
        ...
        for i in range(radius):
            ...
            adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:,0], edges[:,1])), shape=(len(features_batch[i]), len(features_batch[i+1])), dtype=np.float32)    
